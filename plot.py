...
log_list = [
    pd.read_csv("data/example_model/progress.csv"),
]
...
for i in range(len(graphDicts)):
    ...
    for key in logDicts:
        if graphDicts[i] not in logDicts[key]:
            ...
            plt.plot(
                logDicts[key]["misc/total_timesteps"], logDicts[key][graphDicts[i]]
            )

            legendList.append(legends[j])
            print(
                "avg", str(key), graphDicts[i], np.average(logDicts[key][graphDicts[i]])
            )
        j = j + 1
    print("------------------------")
