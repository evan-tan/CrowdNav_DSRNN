...
class SRNNRolloutStorage(object):
    def __init__(
        self,
        num_steps,
        num_processes,
        ...
        if isinstance(obs_shape, dict):
            ...
            for key in obs_shape:
                self.obs[key] = torch.zeros(
                    num_steps + 1, num_processes, *(obs_shape[key].shape)
                )
        ...
        self.recurrent_hidden_states["human_node_rnn"] = torch.zeros(
            ...
            num_processes,
            node_num,
            human_node_rnn_size * double_rnn_size,
        )
        self.recurrent_hidden_states["human_human_edge_rnn"] = torch.zeros(
            num_steps + 1,
            num_processes,
            edge_num,
            human_human_edge_rnn_size * double_rnn_size,
        )
    ...
    def to(self, device):
        ...
        for key in self.recurrent_hidden_states:
            self.recurrent_hidden_states[key] = self.recurrent_hidden_states[key].to(
                device
            )
    ...
    def insert(
        ...
        for key in recurrent_hidden_states:
            self.recurrent_hidden_states[key][self.step + 1].copy_(
                recurrent_hidden_states[key]
            )
    ...
    def after_update(self):
        ...
        for key in self.recurrent_hidden_states:
            self.recurrent_hidden_states[key][0].copy_(
                self.recurrent_hidden_states[key][-1]
            )
    ...
    def feed_forward_generator(
        ...
        for indices in sampler:
            ...
            obs_batch = {}
            for key in self.obs:
                obs_batch[key] = self.obs[key][:-1].view(-1, *self.obs[key].size()[2:])[
                    indices
                ]
            recurrent_hidden_states_batch = {}
            for key in self.recurrent_hidden_states:
                recurrent_hidden_states_batch[key] = self.recurrent_hidden_states[key][
                    :-1
                ].view(-1, self.recurrent_hidden_states[key].size(-1))[indices]
    ...
    def recurrent_generator(self, advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                for key in self.recurrent_hidden_states:
                    recurrent_hidden_states_batch[key].append(
                        self.recurrent_hidden_states[key][0:1, ind]
                    )
