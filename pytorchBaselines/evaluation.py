...
def evaluate(
    ...
    for k in range(test_size):
        ...
        cumulative_rewards.append(
            sum(
                [
                    pow(gamma, t * baseEnv.robot.time_step * baseEnv.robot.v_pref)
                    * reward
    ...
    avg_nav_time = (
        sum(success_times) / len(success_times) if success_times else baseEnv.time_limit
    ...
    logging.info(
        ...
        "nav time: {:.2f}, total reward: {:.4f}".format(
            phase.upper(),
            extra_info,
            success_rate,
            collision_rate,
    ...
    if phase in ["val", "test"]:
        ...
        logging.info(
            ...
            too_close * baseEnv.robot.time_step / total_time,
            avg_min_dist,
        )
